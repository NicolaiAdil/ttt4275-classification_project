...
def display_classification(images, labels, predictions, num_images=5, title="Classification Results"):
    ...
    fig, axs = plt.subplots(nrows=2, ncols=max(num_correct_display, num_incorrect_display), figsize=(15, 6))  # 2 rows, one for correct and one for incorrect
    ...
    for idx in range(num_incorrect_display):
        ...
        ax.imshow(images[image_idx], cmap='gray')
        ax.set_title(f"Incorrect prediction: {predictions[image_idx]}\n(True: {labels[image_idx]})")
        ax.axis('off')  # Hide axes
...
def plot_templates(templates, nTemplates=6, nClasses=10):
    fig, axes = plt.subplots(nClasses, nTemplates, figsize=(12, 20))  # 10 rows for 10 classes
    for i in range(nClasses):  # 10 classes
        for j in range(nTemplates):
            ax = axes[i, j]
            if j < len(templates[i]):  # Check if there's a template to display
                ax.imshow(templates[i][j].reshape(28, 28), cmap='gray')
                ax.set_title(f'Class {i} Template {j+1}', fontsize=10)  # Optionally adjust font size for better fit
            ax.axis('off')  # Hide axes regardless

    fig.suptitle(f"The first {nTemplates}/{len(templates[0])} K-Means Cluster Centers for Each Class", fontsize=16, y=0.95)
