...
def create_train_and_test(setosa_matrix, versicolor_matrix, virginica_matrix, split_index: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    train_setosa_labels, test_setosa_labels = np.zeros(split_index), np.zeros(50 - split_index)

    train_versicolor, test_versicolor = split_dataset(versicolor_matrix, split_index)
    train_versicolor_labels, test_versicolor_labels = np.ones(split_index), np.ones(50 - split_index)

    train_virginica, test_virginica = split_dataset(virginica_matrix, split_index)
    train_virginica_labels, test_virginica_labels = np.full(split_index, 2), np.full(50 - split_index, 2)
